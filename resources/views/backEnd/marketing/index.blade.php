        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{route('sms.create')}}" class="btn btn-primary rounded-pill">Create</a>
                </div>
                <h4 class="page-title">Sms Manage</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
        ...
        <div class="card">
            <div class="card-body">
                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                    ...
                    <tbody>
                        @foreach($Smsteamplete as $key=>$value)
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td> {{$value->type}}</td>
                            <td>{{$value->smsteamplate}}</td>
                            ...
                            <td>
                                ...
                                @if($value->status == 1)
                                    <form method="post" action="{{route('sms.inactive')}}" class="d-inline">
                                    @csrf
                                    ...
                                    @else
                                    <form method="post" action="{{route('sms.active')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
                                    ...
                                    @endif
                                  <a href="{{route('sms.edit',$value->id)}}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                    <form method="post" action="{{route('sms.delete')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
