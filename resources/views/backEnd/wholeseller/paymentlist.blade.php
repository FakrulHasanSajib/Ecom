                    ...
                    <form class="custom_form" id="filterForm">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <select name="whosaler_id" class="form-control w-100 filter-input">
                                    <option value="">Select Wholesaler</option>
                                    @foreach($wholesaler as $whosaler)
                                        <option value="{{$whosaler->id}}" {{ request('order_status') == $whosaler->id ? 'selected' : '' }}>
                                            {{$whosaler->business_name}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="mb-3">
              <label for="payment_method" class="form-label">Whosaler Select</label>
              <select class="form-select" name="whosaler_id">
                <option selected>Select a Whosaler</option>
                @foreach($wholesaler as $whosaler)
                <option value="{{$whosaler->id}}">{{$whosaler->business_name}}</option>
                @endforeach
              </select>
            </div>
        <div class="mb-3">
...
<script>
    ...
    function performFilter() {
        ...
        $.ajax({
            url: "{{ route('wholeseller.payment') }}", // Update with your actual route
            type: 'GET',
            data: formData,
            headers: {
