@extends('backEnd.layouts.master') @section('title','Return Create') @section('css')
<style>
 .increment_btn,
 .remove_btn {
  margin-top: -17px;
 ...
 <div class="row">
  <div class="col-12">
   <div class="page-title-box">
    ...
    <h4 class="page-title">Return To Invoice</h4>
   </div>
  </div>
 </div>
 <!-- end page title -->
   ...
   <div class="card">
    <div class="card-body">
     <form action="{{route('admin.order.store')}}" method="POST" class="row pos_form" data-parsley-validate="" enctype="multipart/form-data">
      @csrf
      <div class="col-sm-12">
          <input type="hidden" name="return_satus" value="11">
    ...
    <input type="number" 
           ...
           placeholder="0.00" 
           data-id="{{$value->rowId}}" 
           data-product-id="{{$value->id}}" {{-- এই অংশটি নতুন যোগ করা হলো --}}
    />
</td>
       ...
       <table class="table table-bordered">
        <tbody id="cart_details">
         @php 
            ...
            // সেশনের বদলে সরাসরি কার্ট থেকে ডিসকাউন্ট বের করা
...
$(document).on("input change", ".product_discount", function (e) {
    var inputField = $(this);
    var id = inputField.data("id"); 
    var product_id = inputField.data("product-id"); // এটি ঠিকমতো আসছে কি না দেখতে হবে
    var discount = parseFloat($(this).val()) || 0;
    
    // --- DEBUGGING START ---
    console.log("--------------------------");
    console.log("Row ID from Input:", id);
    console.log("Product ID from Input:", product_id); // এখানে যদি 'undefined' আসে, তবে HTML এ সমস্যা
    console.log("Discount:", discount);
    
    if(!product_id) {
        alert("Warning: Product ID is missing! Please Inspect Element to check data-product-id attribute.");
    }
    // --- DEBUGGING END ---

    // ভিজ্যুয়াল আপডেট
    var row = $(this).closest('tr');
    var price = parseFloat(row.find('.product_price').val()) || 0;
    var qty = parseFloat(row.find('.cart_update_qty').val()) || 1;
    row.find('td:nth-child(6)').text(((price - discount) * qty).toFixed(2));
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.status === 'success') {
                    console.log("Success! New ID received:", response.new_rowId);
                    
                    // ইনপুট আপডেট
                    inputField.data('id', response.new_rowId);
                    inputField.attr('data-id', response.new_rowId);
